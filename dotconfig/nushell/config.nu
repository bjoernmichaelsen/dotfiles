...
def cdlo [] {
    cd /var/tmp/bjoern/libreoffice
}
def initlo [] {
    let hddlo = $"($env.HOME)/checkouts/core"
    let tmplo = "/var/tmp/bjoern/libreoffice"
    cd $"($hddlo)"
    git fetch github master:githubmaster
    mkdir $"($tmplo)"
    cd $"($tmplo)"
    git clone -b githubmaster $"($hddlo)" .
    git co -b bm/work
    git remote add gerrit ssh://Sweetshark@gerrit.libreoffice.org:29418/core.git
    ./autogen.sh --without-doxygen --without-java --without-system-libxml --without-system-nss --enable-debug --with-external-tar=$"($env.HOME)/.lo-tarballs"
}
alias e = nvim
